...
# DATA SELECTED EVERY 30 SECONDS
...
start=time.time()
df_tropo= {"%s"%receivers[0]+"_"+tf: pd.read_sql_query(""" select epoch_time, data_val , flag_tropo from gnsstropo where flag_tropo=0  and id_tropo_table in 
  ...
  """,engine,params=(tf,receivers[0]))for tf in types }
end=time.time()
...
def get_db (menu_list):
  ...
  for selected_dv in menu_list:
    if selected_dv in imported_rec:
      ...
      df_update= {"%s"%selected_dv+"_"+tf: pd.read_sql_query(""" select epoch_time, data_val ,flag_tropo from gnsstropo where flag_tropo=0 and id_tropo_table in 
        (select id_tropo_table from troposet 
        where type= %s
        and troposet.id_result=(select id_result from ggm where short_name_4ch=%s ) ) order by epoch_time
...
def get_db_points (selectData):
  ...
  for sd in range(int(len(selectData['points']))):
    if selectData['points'][sd]['text'] in imported_rec:
      ...
      df_update= {"%s"%selectData['points'][sd]['text']+"_"+tf: pd.read_sql_query(""" select epoch_time, data_val, flag_tropo from gnsstropo where flag_tropo=0 and id_tropo_table in 
        (select id_tropo_table from troposet 
        where type= %s
        and troposet.id_result=(select id_result from ggm where short_name_4ch=%s ) ) order by epoch_time
...
start=time.time()
df_pos = {"%s"%sn+"_pos": pd.read_sql_query(""" select * from gnssposition where flag_pos=0 and id_pos_table in 
...
def update_rangeslider(selected_dropdown_value,selectData):
  if selectData != None and selectData['points'] !=[]:
    ...
    for sd in range(int(len(selectData['points']))):
      ...
      marks_val = {date.month:{'label':"%s"%date.strftime("%B")+" "+str(date.year),'style':{'color':'#ff6600','font-size':'120%','font-weight': 'bold'}} for date in df_tropo["%s"%selectData['points'][sd]['text']+"_ZTD"]['date'].dt.date.unique()}
